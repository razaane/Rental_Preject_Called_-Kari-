...
// Fetch rental data
$existing = $rental->findById((int)$rental_id);
if (!$existing) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($rental->update((int)$rental_id, $data)) {
...
<div class="max-w-2xl mx-auto bg-surface-light dark:bg-surface-dark p-8 rounded-xl shadow-soft">
    <h1 class="text-2xl font-bold text-primary mb-4">Edit Rental</h1>
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="flex flex-col">
            ...
            <textarea required name="descreption" rows="4" placeholder="Describe the property details..."
                class="w-full bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl p-3 focus:ring-2 focus:ring-primary focus:border-primary transition"><?= htmlspecialchars($existing['descreption']) ?></textarea>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="flex flex-col">
                ...
                <input required type="text" name="adress" placeholder="123 Luxury Ave"
                    value="<?= htmlspecialchars($existing['adress']) ?>"
                    class="w-full bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl p-3 focus:ring-2 focus:ring-primary focus:border-primary transition"/>
            </div>
            <div class="flex flex-col">
                ...
                <input required type="text" name="city" placeholder="San Francisco"
                    value="<?= htmlspecialchars($existing['city']) ?>"
                    class="w-full bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl p-3 focus:ring-2 focus:ring-primary focus:border-primary transition"/>
            </div>
        </div>
        ...
        <div class="flex flex-col">
            ...
            <input required type="url" name="img_url" placeholder="https://..."
                value="<?= htmlspecialchars($existing['img_url']) ?>"
                class="w-full bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl p-3 focus:ring-2 focus:ring-primary focus:border-primary transition"/>
        </div>
        ...
        <div class="flex flex-col">
            ...
            <input required type="text" name="available_dates" placeholder="2024-10-12 to 2024-10-20"
                value="<?= htmlspecialchars($existing['available_dates']) ?>"
                class="w-full bg-background-light dark:bg-background-dark border border-border-light dark:border-border-dark rounded-xl p-3 focus:ring-2 focus:ring-primary focus:border-primary transition"/>
        </div>
